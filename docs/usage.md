...
Supermind 是对软件项目结果负责的独立 Agent，当前通过插件挂载 Codex。直接描述目标或继续已有工作；它承接必要的项目认识，在授权范围内推进，根据反馈调整，并把重要变化更新到原有材料。记忆与多端同步沿用已有 Git 机制。

能力在真实使用中生长，不以预设演练或验证通过为前提。用户明确无需验证时，不额外安排测试或审查。
...
> 原先的假设变了，我们先探索现有接口能否满足需求。

> 这次修复形成了一条工程经验，先评估是否值得抽象，不写能力库。

软件生命周期用于理解情境，不是必须走完的流程。AI 根据已有材料与你协作，解释、探索或试验都可以是本次成果。非软件任务不进入 Supermind；一般讨论、解释和局部修改不要求先初始化能力库。

核心决策根据生命周期和具体场景选择下一步。抵达场景规定的检索时点时，先查询相关需求，再判断候选和采用方式；候选沉淀和每次具体复用都要由人确认。工程实践需要准确摘要、适用情境、真实载体和收益依据，不必变成技能或代码。场景上下文通过现有需求的 intent 与 contract 传给 CLI，不新增宿主监听器。不支持的载体不会伪装成代码入库。

## 能力库如何工作

独立的实践步骤与决策原则可以作为“方法”（`method`）保存。分类描述解决的问题，载体描述如何采用：例如组件库直出原型属于 `design` 分类、`method` 载体。正文包含触发条件、必要输入、步骤、差异配置、适用边界和完成依据，可由库内共享文档独立承载；模板与可执行技能分别使用 `template` 与 `skill`。
...
顶级分类为代码、产品、设计、工程、工具和数据，负责组织目录。总览与分类页统一使用“名称、说明”两列；名称采用简洁的“中文名（英文名）”，说明使用较小字号。详情页展示用途、官方入口、触发场景、接入方式和必要边界，不展示状态管理、来源版本或内部评分。
...
源码修改、远端发布与本地插件更新分别按用户授权推进；本轮不发布、不更新已安装插件。开发目录安装仅用于明确需要的本地开发。
