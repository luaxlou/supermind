...
本稿用于确认核心设计，尚未替换现行技能和运行时行为。下图中的决策职责与调用关系是拟落实的执行设计；现有实现与待改造部分在“落地范围与真实能力边界”中分别说明。
...
现行实现已经有生命周期名称、协作原则和 `begin-design` 检索入口，但缺少场景目录与该入口之间的明确连接；本文确认后，需要同步改造以下部分：
...
```mermaid
flowchart TB
    subgraph Change[需要改造的核心入口与场景指导]
        A[SKILL.md：收到任务或新反馈]
        B[核心决策指引：判断下一步]
        C[场景指引：进入条件与动作要求]
        D[能力调用指引：选择实际入口]
...
图中的核心决策指引与场景指引是待实现的设计位置。入口文件必须实际引用并执行它们；产品设计稿只解释机制，不会自行改变 Agent 行为。现有 `search` 用于查询，`begin-design` 还会执行项目来源刷新及后续决策流程，调用时继续遵循用户限定的读写范围，不能把所有场景机械送入同一入口。
...
本稿首先确认核心决策循环、生命周期场景覆盖和人机边界；确认后再制定具体入口与工具改造。
