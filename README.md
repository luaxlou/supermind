...
Supermind 是面向 Codex 的产品开发协作工具。你只需说明期望的结果，它会理解现有产品和代码，在保持产品连续性的前提下，组织合适的技能与工具完成开发。遇到可能重复建设的模块时，它会先检索能力库，评估已有实现是否值得抽象或复用。

Superpowers 只是可选的能力来源之一，不是 Supermind 的运行基础，也不是安装前提。

## 能做什么

- 从一个有价值的完整使用场景开始建设新产品。
- 在现有产品上增加功能、调整行为、修复问题、改善质量、重构和发布。
- 根据当前需要选择技能和工具，而不是要求用户学习工作流命令。
- 从当前项目、已登记项目以及已安装的技能和插件中发现候选能力。
- 评估能力的通用边界、契约匹配、验证证据、接入成本、维护风险和预期收益。
- 每次具体复用或适配前取得人工确认；入库、得分高或过去用过都不代表获得授权。
- 记录复用结果和未满足的需求，为后续决策提供依据。
- 通过分类目录、能力详情、决策说明和 Mermaid 关系图展示能力库。
...
必要性、摘要准确性、通用边界和具体复用仍需人工审查确认。详见[能力库建设与自举验收](docs/product/capability-memory-bootstrap.md)。
