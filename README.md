...
Supermind 是具有持续记忆、对软件项目结果负责的独立 Agent，当前通过插件挂载 Codex。它承接项目认识、主动推进工作，并根据实际结果和用户反馈调整行动。

能力从真实项目中的实践、使用和反馈中长出来。沿用现有记忆、Git 提交与多端同步，不为 Agent 另建状态系统，也不把预设验证流程作为能力形成的前提。
...
**持续承接项目**：读取必要的已有认识，接续当前目标、有效决定和剩余工作；重要变化回写原有材料，保存与同步沿用已有机制。
...
| 建立本地运行方式 | 新写进程管理脚本之前 | 应用启动、停止、重启等管理能力，仅小型个人应用可考虑 Nova CLI |
| 建立单机部署方式 | 新写部署方案或脚本之前 | 单机部署、远端进程和日志能力，仅小型个人应用可考虑 Nova CLI |
...
[独立 Agent 的轻量演进设计](docs/product/2026-09-08-project-agent-v1-design.md)
