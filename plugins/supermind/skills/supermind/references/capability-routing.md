...
详情页直接展示用途、官方项目入口、触发场景、接入方式与必要边界，只保留一个页面标题。不用“契约”等元数据栏目包裹完整说明，不展示内部评分，不生成空依赖、空使用方等栏目。实际验证记录按需折叠。目录和详情均复用 GitHub 与 CLI 的同一呈现逻辑；交付前读取生成后的完整页面，不能只检查字段或同步结果。
