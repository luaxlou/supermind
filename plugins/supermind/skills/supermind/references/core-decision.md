...
设计阶段按[持续语境指引](design-stage.md)在进入时加载需求基线和 UI 基线，后续循环以它们为语境。重新判断当前动作不等于重新查询这两项方法；业务决定变化时更新项目基线，不把阶段内每个动作变成能力检索事件。
...
用户要求沉淀可复用经验时，完成条件采用[抽象审查](abstraction-review.md)：交付可独立采用的正文或资产，以及针对它的迁移推演和限制。不能将任务降格成来源整理或候选登记。用户纠正能力复用机制时，检查共享技能、模板及生成卡，不只修正当次入库记录；机器检查通过不等于语义审查通过。
