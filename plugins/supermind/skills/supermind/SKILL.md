...
先阅读[生命周期与核心范式](references/actions.md)。软件生命周期是预先定义的顶层坐标，不是顺序执行的流水线；除此之外，场景和协作方式都是可扩展能力。

根据当前意图、所处情境和相关材料开展人机交互。AI 主动读取已有资料、组织证据和承担常规工作；用户参与价值取舍和关键判断，不必维护工作流。询问、解释、探索、评估、试验或复盘都可以构成完整成果，不强行收敛为方案、代码或决策。

新信息改变方向时，指出它影响的判断，并依据最新对话继续；变化含义不清楚时聚焦澄清。没有必要把用户拉回旧流程，也不因一次分支讨论重新启动整个流程。已有约束是否变化应有依据，授权不能从话题变化中推导。

按[能力调用指引](references/capability-routing.md)选择适用方法。核心范式提供触发线索、相关材料和协作目的，不是固定动作清单。具体工具的安全与协议约束仍须遵守。

## 需要使用能力库时

明确查询或管理能力库，或当前工作已进入核心复用范式中的库检索、登记或结果记录时，才启动能力库。一般讨论、解释、局部修改和仅基于已提供材料的候选评估，不以能力库配置为前提。这是适用范围划分，不是检索失败后的替代路径。
